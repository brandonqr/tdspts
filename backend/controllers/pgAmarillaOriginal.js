...
class PgAmarilla {
    ...
    BuscarEnWeb(busqueda) {
            ...
            this.nightmare
                ...
                .catch(error => {
                    console.log("Ha petado al inicio ");
    ...
    ObtenerLinksUrlResultados() {
        this.nightmare
            ...
            .then(links => {
                ...
                this.ObtenerObjeto();
                console.log(
                    `Guardando link ${this.contador + 1} de ${this.todosLosLinks.length}`
                );
                this.contador++;
            })
            .catch(error => {
                console.log('Ha petado en ObtenerLinkResultados')
                console.error("Search failed:", error);
            });
    }
    ObtenerObjeto() {
        this.nightmare
            ...
            .then(web => {
                this.web = web;
                if (
                    this.contadorTodosLosLinks < this.todosLosLinks.length - 1 ||
                    this.todosLosLinks.length == 1
                ) {
                    if (
                        this.contadorObjeto < this.links.length - 1 ||
                        this.todosLosLinks.length == 1
                    ) {
                        this.InsertarEnDB();
    ...
    ActualizarDatos() {
        this.nightmare
            ...
            .catch(error => {
                console.log("Ha petado en Actualizar datos");
                console.error("Search failed:", error);
            });
    }
    GenerarLinks(url) {
